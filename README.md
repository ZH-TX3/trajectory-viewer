...
- **配置统一管理（Config Hub）** — 顶栏切换进入，统一管理各工具的 skills / agents / 提示词 / MCP / 配置文件：
  - **一处存储，处处生效** — skills 与 agents 统一存放在 `~/.agents/`，按工具软链接分发
  - **逐工具开关** — 每个资源对每个工具独立开关；软链接失败时自动回退为目录复制
  - **收编漂移副本** — 扫描发现各工具目录里的散落实体副本，逐个预览确认后收编进统一存储
  - **可恢复删除** — 删除资源时先解除所有链接，再移入回收站，可随时恢复
  - **只读展示** — 提示词与各工具配置文件（JSON / TOML / YAML）集中展示
